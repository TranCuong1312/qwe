...
BEGIN
    SET @CURDATE = CURDATE();
	SELECT MODULEID INTO MODULE_STILL_ON_WORK
    FROM MODULES M
    JOIN WORKDONES W ON M.MODULEID=W.MODULEID
    WHERE @CURDATE BETWEEN M.PROJECTMODULESTARTDATE AND W.WORKDONEDATE;
END $$
...
DELIMITER $$
CREATE PROCEDURE WHATHISFACE(OUT WHATHISFACE TINYINT)
BEGIN
	SELECT E.EMPLOYEEID INTO WHATHISFACE
    FROM EMPLOYEES E
